...
INNER JOIN productos ON productos.id = productos_vendidos.id_producto WHERE ventas.tipo_pago ='Tarjeta'
AND ventas.fecha  > DATE_SUB(NOW(), INTERVAL 1 DAY)
GROUP BY ventas.id ORDER BY ventas.id DESC;");
// ORIGINAL WHERE   = 0 and ventas.fecha BETWEEN CURRENT_DATE()-7 AND CURRENT_DATE()+1  GROUP BY ventas.id ORDER BY ventas.id DESC;");
