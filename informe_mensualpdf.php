...
INNER JOIN productos ON productos.id = productos_vendidos.id_producto WHERE ventas.tipo_pago ='Efectivo' and ventas.fecha  > DATE_SUB(NOW(), 
INTERVAL 1 MONTH) AND CURRENT_DATE()+1  GROUP BY ventas.id ORDER BY ventas.id;");
...
foreach($ventas as $venta){
    ...
    $pdf->Ln(10);
        ...
        $pdf->cell(85,10,utf8_decode('Descripción'),1,0,'C',1);
        ...
        foreach(explode("__", $venta->productos) as $productosConcatenados){ 
            ...
            $pdf->cell(85,10,utf8_decode($producto[1]),1,0,'C',1);
